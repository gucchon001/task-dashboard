...
# -*- coding: utf-8 -*-
"""
統合PC（WIN-ND0QPM2D7G1）専用接続テストスクリプト
WIN-ND0QPM2D7G1とのWinRM接続をテストします。
"""
...
def test_integration_connection():
    """WIN-ND0QPM2D7G1との接続をテストする"""
    
    print("=== WIN-ND0QPM2D7G1（統合PC）接続テスト開始 ===")
    ...
    # WIN-ND0QPM2D7G1の情報を取得
    integration_pc = None
    for pc in config.get('pcs', []):
        if pc.get('name') == 'WIN-ND0QPM2D7G1':
            integration_pc = pc
            break
    
    if not integration_pc:
        print("❌ WIN-ND0QPM2D7G1の設定が見つかりません。")
        return
    
    pc_name = integration_pc.get('name')
    pc_ip = integration_pc.get('ip')
...
if __name__ == "__main__":
    print("WIN-ND0QPM2D7G1（統合PC）接続テストスクリプトを開始します...")
